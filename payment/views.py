    ...
    if payment_status == 'OK':
        ...
        if 'data' in response.json() and (
            ...
            if payment_code == 100:
                ...
                order.save()
                messages.success(request, 'your payment was success')
                return redirect('home')

            elif payment_code == 101:
                messages.warning(request, 'this payment is duplicate and its already been done successfully')
                return redirect('home')

            else:
                error_code = response.json()['errors']['code']
                error_msg = response.json()['errors']['message']
                messages.error(request, f'failure in payment-{error_code}-{error_msg}')
                return redirect('home')
    else:
        messages.error(request, f'failure in payment')
        return redirect('home')
